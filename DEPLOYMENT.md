...
| `05_add_company_notes.sql`| Adds a `notes` column to `companies` (superseded by 06 below — harmless leftover, safe to run or skip) |
| `06_company_notes_table.sql` | Adds the `company_notes` table backing the new Notes tab (chronological, multi-entry notes per company) |
| `docket.html`             | The complete, updated app, now with email/password login, Company Details, Notes, and Search/Sort/Filter |
...
6. Paste the full contents of `06_company_notes_table.sql` → **Run**.
   This is what the Notes tab actually reads and writes to — without it,
   creating a note will fail. (`05_add_company_notes.sql` is an earlier,
   superseded approach kept only for history; running it is harmless but
   not required.)
